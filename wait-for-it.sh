...
wait_for_wrapper()
{
    ...
    if [[ $WAITFORIT_QUIET -eq 1 ]]; then
        timeout $WAITFORIT_TIMEOUT $0 --quiet --child --url=$WAITFORIT_URL--timeout=$WAITFORIT_TIMEOUT &
    else
        timeout $WAITFORIT_TIMEOUT $0 --child --url=$WAITFORIT_URL --timeout=$WAITFORIT_TIMEOUT &
    fi
    WAITFORIT_PID=$!
